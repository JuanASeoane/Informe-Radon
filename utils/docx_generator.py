...
class ReportContext:
    ...
    logos: list = field(default_factory=list)  # lista de bytes de imagen, hasta 3
...
def _build_header(section, ctx: ReportContext):
    ...
    widths = [Cm(4), Cm(4), Cm(4), Cm(5)]
    for cell, w in zip(table.rows[0].cells, widths):
        cell.width = w
        _set_cell_borders(cell)

    logo_cells = table.rows[0].cells[:3]
    for i, cell in enumerate(logo_cells):
        cell.vertical_alignment = 1
        para = cell.paragraphs[0]
        para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        if i < len(ctx.logos) and ctx.logos[i]:
            try:
                run = para.add_run()
                run.add_picture(io.BytesIO(ctx.logos[i]), height=Cm(1.6))
            except Exception:
                para.add_run("[logotipo]").italic = True
        else:
            placeholder = ["UPRL", "SERVIZO GALEGO DE SAÚDE", "ÁREA SANITARIA"][i]
            r = para.add_run(placeholder)
            r.font.size = Pt(7)
            r.italic = True

    data_cell = table.rows[0].cells[3]
...
def _section_1(doc, ctx: ReportContext):
    ...
    merged01.text = f"ÁREA SANITARIA DE {ctx.xerencia}" if ctx.xerencia else ""
    ...
    cells2[0].text = "ENDEREZO"
    cells2[0].paragraphs[0].runs[0].bold = True
    merged2 = cells2[1].merge(cells2[2]).merge(cells2[3])
    merged2.text = ctx.enderezo

    cells3 = table.rows[3].cells
    cells3[0].text = "DESCRICIÓN DO CENTRO"
    cells3[0].paragraphs[0].runs[0].bold = True
    cells3[1].text = f"Superficie construída: {ctx.superficie_construida}"
    cells3[2].text = f"Superficie útil: {ctx.superficie_util}"
    cells3[3].text = f"N.º plantas: {ctx.num_plantas}"
...
def _section_7(doc, df: pd.DataFrame, ctx: ReportContext):
    ...
    for _, row in df.iterrows():
        codigo_zona = _row_value(row, "Código Sala", "Sala")
        ...
        puestos = _row_value(row, "Puestos en la sala", default="")
    ...
    if not df.empty:
        ...
        if any_exceeds:
            ...
            for _, row in df[mask].iterrows():
                sala = _row_value(row, "Sala", "Código Sala", default="")
                if sala:
