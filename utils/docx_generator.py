...
from utils.excel_parser import _strip_accents, traducir_es_gl

NIVEL_REFERENCIA_BQ_M3 = 300

# Áreas para las que el informe se redacta "en general" (sin mencionar
# unidade/servizo): atención primaria, PAC, atención especializada o cuando
# no hay área indicada. Cualquier otro valor de área (un servizo/unidade
# concreto, p.ej. "Radioloxía") activa la redacción alternativa.
AREAS_GRUPO_XERAL = {"", "atencion primaria", "pac", "atencion especializada"}


def _area_grupo(area: str) -> str:
    """'A' (xeral: primaria/PAC/especializada/baleiro) ou 'B' (servizo ou
    unidade concreta)."""
...
def _add_field(paragraph, field_code: str, cached_text: str = "1", font_size_pt: float = 9):
    """Inserta un campo simple de Word (PAGE, NUMPAGES) usando <w:fldSimple>.

    Es la forma de campo más sencilla y la mejor soportada de forma
    consistente por Word, LibreOffice/OpenOffice y Google Docs para campos
    únicos y no anidados como estos. La alternativa con fldChar
    begin/separate/end (campo "complejo") es más flexible pero LibreOffice,
    en modo de edición interactivo, a veces no la recalcula y muestra en su
    lugar el nombre genérico del tipo de campo."""
...
def _build_header(section, ctx: ReportContext):
    ...
    # Importante: hay que fijar el ancho tanto en la rejilla de columnas de
    # la tabla (tblGrid/gridCol) como en cada celda (tcW); si solo se pone en
    # la celda, la rejilla sigue marcando columnas iguales y algunos
    # procesadores (Word, LibreOffice) la usan a ella, partiendo la cabecera
    # por la mitad pese al ancho de celda indicado.
    ...
    centro_txt = ctx.centro.upper() if ctx.centro else "..................."
    if _area_grupo(ctx.servizo_unidade) == "A":
        titulo_texto = f"INFORME DE RESULTADOS MEDICIÓNS DE RADON NO {centro_txt}"
    else:
        area_txt = ctx.servizo_unidade.upper() if ctx.servizo_unidade else "..................."
        titulo_texto = (
            f"INFORME DE RESULTADOS MEDICIÓNS DE RADON NA UNIDADE / SERVIZO DE "
            f"{area_txt}, NO {centro_txt}"
        )
...
def _section_1(doc, ctx: ReportContext):
    ...
    cells4[0].text = "DESCRICIÓN DA ÁREA MOSTREXADA"
...
def _section_2(doc, ctx: ReportContext):
    ...
    centro_txt = ctx.centro or "................."
    if _area_grupo(ctx.servizo_unidade) == "A":
        lugar = centro_txt
    else:
        area_txt = ctx.servizo_unidade or "................."
        lugar = f"Servizo / Unidade de {area_txt}, no {centro_txt}"
    _body(
        doc,
        f"O presente informe ten como obxecto recoller os resultados das medicións de radon "
        f"no {lugar}, para, no caso de que a "
...
def _section_3(doc, ctx: ReportContext):
    ...
    if grupo == "A":
        intro_suxeito = centro_txt
    else:
        area_txt = ctx.servizo_unidade or "....."
        intro_suxeito = f"Servizo / Unidade de {area_txt} no {centro_txt}"
    _body(
        doc,
        f"O {intro_suxeito}, consta dos seguintes postos de "
        f"traballo e a seguinte ocupación por espazo de traballo, susceptibles de exposición a "
        f"radón pola súa situación en planta baixa ou baixocuberta:",
    )
    ...
    if grupo == "A":
        cierre_texto = f"Os traballadores/as do {centro_txt} foron informados da "
    else:
        area_txt = ctx.servizo_unidade or "....."
        cierre_texto = (
            f"Os traballadores/as da unidade/servizo {area_txt} no {centro_txt}, foron informados da "
        )
    _body(
        doc,
        f"{cierre_texto}"
...
def generar_conclusion_automatica(df: pd.DataFrame) -> str:
    """Texto de conclusiones que se generaría automáticamente (punto 8) a
    partir de la tabla de resultados. Se expone aparte para poder mostrarlo
    como valor por defecto, editable, en la app."""
