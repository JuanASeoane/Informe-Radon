...
def componer_plano(imagen_bytes: bytes, puntos: list[dict]) -> bytes:
    ...
    for punto in puntos:
        ...
        # fondo blanco semitransparente para que el texto se lea sobre el plano
        draw.rectangle(
            [cx - 2, cy - 1, cx + bw + 2, cy + bh + 1],
            fill=(255, 255, 255),
            outline=(220, 0, 0),
        )
        draw.multiline_text((cx, cy), "\n".join(lineas), font=font, fill=(20, 20, 20), spacing=2, align="center")
...
def generar_documento_anexo2(
    ...
    """Genera un .docx con una página por cada plano que tenga detectores
    asociados: el plano con los puntos marcados a la izquierda, y el
    logotipo + foto exterior del centro a la derecha, sin tapar el plano."""
    planos = datos["planos"]
    ...
    for i, clave in enumerate(nombres_con_puntos):
        ...
        col_plano_cm, col_lateral_cm = 14.0, 4.0
        ...
        w_plano, h_plano = _fit(compuesto, col_plano_cm - 0.3, 21.0)
        ...
        if logo_bytes:
            r = p_lat1.add_run()
            w_logo, h_logo = _fit(logo_bytes, col_lateral_cm - 0.4, 3.0)
            r.add_picture(io.BytesIO(logo_bytes), width=w_logo, height=h_logo)
        else:
        ...
        if foto_exterior:
            ...
            p_lat2.paragraph_format.space_before = Pt(12)
            r2 = p_lat2.add_run()
            w_foto, h_foto = _fit(foto_exterior, col_lateral_cm - 0.4, 8.0)
            r2.add_picture(io.BytesIO(foto_exterior), width=w_foto, height=h_foto)
            cap = celda_lateral.add_paragraph()
            cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
            cap_run = cap.add_run("Foto exterior do centro")
            cap_run.italic = True
