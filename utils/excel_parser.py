...
def postos_traballo_bullets(df: pd.DataFrame) -> list[str]:
    ...
    for _, row in df.iterrows():
        ...
        if pd.isna(sala):
            continue
        sala = traducir_es_gl(str(sala).strip())
...
def categorias_turnos_bullets(
    df: pd.DataFrame, categorias_df: pd.DataFrame | None
) -> tuple[int, list[str]]:
        ...
        turnos = sorted(
            {
                ...
                if _profession_root(name) == root and _traducir_turno(t)
            }
        )
        turno_text = " / ".join(turnos)
        categoria_gl = traducir_es_gl(str(categoria).strip())
        if turno_text:
            bullets.append(f"{categoria_gl}: {n} (turno de {turno_text})")
        else:
            bullets.append(f"{categoria_gl}: {n}")
