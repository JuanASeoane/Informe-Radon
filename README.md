...
│   └── assets.py              # Carga del logotipo institucional por defecto
├── assets/
│   └── logo_uprl.png          # Logotipo por defecto (UPRL / SERGAS / Área Sanitaria)
├── requirements.txt
...
automáticamente el Anexo II. En el paso 5, elige **"Generar automáticamente
a partir del Excel"** o **"Subir mi propio archivo"** — son excluyentes,
así que puedes optar por subir tu propio documento sin que se genere nada
automáticamente.

Al generarlo, se crea un `.docx` en **horizontal**, con una página por cada
plano distinto que tenga detectores asociados:
...
- El logotipo y la foto exterior del centro en una columna aparte (sin
  tapar el plano), con el nombre del centro debajo de la foto.
...
incluye en el ZIP y en el PDF completo igual que el resto de anexos.

## Logotipo por defecto

Si no subes ningún logotipo propio, tanto la cabecera del informe como el
Anexo II usan automáticamente el logotipo institucional incluido en
`assets/logo_uprl.png` (UPRL / Servizo Galego de Saúde / Área Sanitaria).
Para cambiarlo, sustituye ese fichero por otro con el mismo nombre — no
