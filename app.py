...
def verificar_conexion_supabase():
    """Verifica que la conexión a Supabase funciona con más detalles de error."""
    if not SUPABASE_AVAILABLE:
        st.warning("⚠️ Supabase no disponible. Instala las dependencias.")
        return False
    
    try:
        # Obtener URL y KEY para diagnóstico
        try:
            url = st.secrets["connections.supabase"]["SUPABASE_URL"]
            key = st.secrets["connections.supabase"]["SUPABASE_KEY"]
        except KeyError as e:
            st.error(f"❌ Error en secrets.toml: Falta la clave {e}")
            return False
