...
st.caption(
    ...
    "el informe se creará también un ZIP con el informe Word y los anexos juntos."
)
...
for key, label in ANEXOS_INFO:
    anexos_files[key] = st.file_uploader(
...
if st.button("📄 Generar informe Word", type="primary"):
    ...
    anexos_subidos = {k: f for k, f in anexos_files.items() if f is not None}
    if anexos_subidos:
        zip_buffer = io.BytesIO()
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(report_name, buffer.getvalue())
            for i, (key, _) in enumerate(ANEXOS_INFO, start=1):
                f = anexos_subidos.get(key)
                if f is not None:
                    ext = f.name.rsplit(".", 1)[-1] if "." in f.name else "pdf"
                    numero = ["I", "II", "III", "IV"][i - 1]
                    zf.writestr(f"ANEXO_{numero}.{ext}", f.getvalue())
