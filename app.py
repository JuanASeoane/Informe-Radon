...
for key, label in ANEXOS_INFO:
    if key == "anexo2":
        st.markdown(f"**{label}**")
        st.caption(
            "Puedes generar este plano automáticamente a partir de las imágenes y "
            "coordenadas (Punto X / Punto Y) del Excel, o subir tu propio archivo abajo."
        )
        if st.button("🗺️ Generar plano automáticamente a partir del Excel", key="generar_anexo2"):
            try:
                datos_planos = extraer_datos_planos(uploaded)
                anexo2_docx = generar_documento_anexo2(datos_planos, logo_bytes=logo_bytes)
