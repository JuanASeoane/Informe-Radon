...
with col3:
    postos_traballo_desc = st.text_area(
        "Descripción de los puestos de trabajo (autorrellenado con las salas medidas)",
        value=", ".join(salas),
    )
    num_traballadores_default = (
        f"{total_personas} ({categorias_texto})" if categorias_texto else ""
    )
    num_traballadores = st.text_input(
        "N.º de trabajadores adscritos (autorrellenado con la hoja de categorías profesionales)",
        value=num_traballadores_default,
    )
